...
|요소|의미|예시|
|---|---|---|
|태그(tag)|'<'와'>'로 둘러싸인 문자열 시작 태그 '<>'와 종료태그 '</>'로 구성|**<title>** HTML **</title>**|
|내용(content)|태그로 둘러싸인 문자열|<title>**HTML**<title>|
|엘리먼트(element)|태그와 내용을 포함한 전체 문자열 HTML 문서의 기본 구성 단위|**<title> HTML <title>**|
|속성(arrtibute)|엘리먼트의 상세한 표현 설정 사항을 지시 시작 태그 안에 사용|<title **color=**"black"> HTML </title>|
|속성값(value)|속성값('' 또는 ""로 감싸야 함)|<title color=**"black"**> HTML </title>|
